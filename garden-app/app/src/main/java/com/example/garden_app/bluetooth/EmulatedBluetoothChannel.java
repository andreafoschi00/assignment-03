    ...
    private class TcpWorker implements ExtendedRunnable {
        ...
        TcpWorker(Socket socket) {
            ...
            try {
                ...
                Log.e(com.example.garden_app.utils.C.LIB_TAG, "Error occurred when creating input stream", e);
            }
            try {
                tmpOut = socket.getOutputStream();
            } catch (IOException e) {
                Log.e(com.example.garden_app.utils.C.LIB_TAG, "Error occurred when creating output stream", e);
            }
            ...
            while (true) {
                try {
                    ...
                    while ((inputByte = input.readByte()) != 0) {
                        ...
                        if(chr != com.example.garden_app.utils.C.message.MESSAGE_TERMINATOR){
                            readbuffer.append(chr);
                        } else {
                            String inputString = readbuffer.toString();
                            Message receivedMessage = getBTChannelHandler().obtainMessage(
                                    com.example.garden_app.utils.C.channel.MESSAGE_RECEIVED,
        ...
        public void write(byte[] bytes) {
            try {
                ...
                bytesToBeSent[bytesToBeSent.length -1] = (byte) com.example.garden_app.utils.C.message.MESSAGE_TERMINATOR;

                outputStream.write(bytesToBeSent);

                Message writtenMsg = getBTChannelHandler().obtainMessage(
                        com.example.garden_app.utils.C.channel.MESSAGE_SENT,
                        -1,
                        -1,
                        bytes
                );
        ...
        public void cancel() {
            try {
                ...
                Log.e(com.example.garden_app.utils.C.LIB_TAG, "Could not close the connect socket", e);
            }
        }
    }
}
